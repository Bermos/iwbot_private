...
public class ListenerED extends Listener {
    @Override
    public void onGuildJoin (GuildJoinEvent event) {
        GuildHandler.newGuild(event.getGuild().getId(), event.getGuild().getOwner().getUser().getId(), event.getGuild().getOwner().getEffectiveName());
    ...
    public void onGuildMemberJoin (GuildMemberJoinEvent event) {
        if (GuildHandler.isAutoWelcome(event.getGuild().getId())) {
            GuildHandler.welcome(event.getJDA(), event.getGuild().getId(), event.getMember().getUser().getId());
        }
        else {
            GuildHandler.joined(event.getMember().getUser().getId(), event.getGuild().getId());
        }
    }
}
